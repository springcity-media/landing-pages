...
beforeEach(async () => {
  ...
  await environment.withSecurityRulesDisabled(async (context) => {
    ...
    await setDoc(doc(database, "landingPages/published"), {
      slug: "welcome",
      status: "published",
    });
    await setDoc(doc(database, "landingPages/draft"), {
      slug: "secret",
      status: "draft",
    });
...
test("allows a published landing page to be read", async () => {
  ...
  const snapshot = await assertSucceeds(
    getDoc(doc(database, "landingPages/published")),
  );
...
test("denies all client writes", async () => {
  const database = environment.unauthenticatedContext().firestore();
  await assertFails(
    setDoc(doc(database, "landingPages/new"), { status: "published" }),
  );
});
