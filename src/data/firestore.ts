...
function readOnlyConverter<T>(read: (id: string, data: DocumentData) => T): FirestoreDataConverter<T> {
  return {
    toFirestore: () => {
...
const sponsorConverter = readOnlyConverter<Sponsor>((id, data) => ({
  ...
  sortOrder: (() => {
    ...
    if (typeof value !== "number") throw new Error("Invalid Firestore record: sortOrder must be a number.");
    return value;
  })(),
}));
...
export class FirestoreContentRepository {
  ...
  async getEvent(id: string): Promise<Event | null> {
    const snapshot = await getDoc(doc(this.database, "events", id).withConverter(eventConverter));
    return snapshot.data() ?? null;
  }

  async getLandingPageBySlug(slug: string): Promise<LandingPage | null> {
    const pages = collection(this.database, "landingPages").withConverter(landingPageConverter);
    const snapshot = await getDocs(query(pages, where("slug", "==", slug), where("status", "==", "published"), limit(1)));
    return snapshot.docs[0]?.data() ?? null;
  }

  async getSponsors(eventId: string): Promise<Sponsor[]> {
    const sponsors = collection(this.database, "sponsors").withConverter(sponsorConverter);
    const snapshot = await getDocs(query(sponsors, where("eventId", "==", eventId), where("status", "==", "published")));
    return snapshot.docs.map((item) => item.data()).sort((a, b) => a.sortOrder - b.sortOrder);
  }
}
